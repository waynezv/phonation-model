...
glottal_flow = (glottal_flow - np.min(glottal_flow)) / (np.max(glottal_flow) - np.min(glottal_flow))
...
while np.linalg.norm(R) > 1:  # norm of the difference between predicted and real glottal flow
    ...
    # plt.figure()
    # plt.subplot(211)
    # plt.plot(range(int(0.1 * fs)), glottal_flow[:int(0.1 * fs)], 'b.-')
    # plt.subplot(212)
    # plt.plot(sol[:int(0.1 * fs), 0], u0[:int(0.1 * fs)], 'b.-')
    # plt.xlabel('t')
    # plt.title('volume velocity')
    # plt.show()
    ...
    adjoint_sol = dae_solver(residual, M_T, dM_T, T,
                             ...
                             display_progress=True, report_continuously=True, verbosity=50)
    ...
    d_delta = np.sum(0.5 * (X[:, 1] - X[:, 0]))

    # TODO: stepsize, adaptive adjust stepsize
    alpha = alpha - 0.01 * d_alpha
    beta = beta - 0.01 * d_beta
    delta = delta - 0.01 * d_delta
